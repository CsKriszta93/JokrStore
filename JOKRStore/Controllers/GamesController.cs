...
namespace JOKRStore.Web.Controllers
{
    public class GamesController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            games.new_release = gameDtos.Where(x => x.State == 0).OrderBy(x => x.Release).Take(6).Select(x => mapper.Map<GameViewModel>(x));
            games.new_tests = gameDtos.Where(x => x.State == 1).OrderBy(x => x.Release).Take(6).Select(x => mapper.Map<GameViewModel>(x));
